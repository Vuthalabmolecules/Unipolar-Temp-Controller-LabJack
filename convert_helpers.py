...
R2 = 9977 #Value of other resistor. May need to change if changing resistor.
...
# Functions for the first Unipolar controller 
def get_temp():
...
# Functions for the second Unipolar controller 
def get_temp2():
    """Returns temperature in Degrees Celsius"""
    #Calculates thermistor resistance
    vain0 = ljm.eReadName(handle, "AIN2") #Voltage divider voltage
    vain1 = ljm.eReadName(handle, "AIN3") #Total positive to negative voltage
    resistance = float(vain0/(vain1-vain0))*R2 #Thermistor resistance calculation
    #Code for thermistor temperature calculation. Constants above and below as well as equation from https://www.thorlabs.com/_sd.cfm?fileName=4813-S01.pdf&partNumber=TH10K
    logarith = np.log(resistance/10000.0) #Logarithm required
    T = 1.0/(a + b * logarith + c * (logarith ** 2) + d * (logarith ** 3)) #Temperature calulcation
    return (T - 273.15)
...
def set_temp2(temp):
    ...
    Rth = (10000.0)*np.exp(A + B/tempC + C/(tempC ** 2) + D/(tempC ** 3)) #Thermistor resistance
    #Voltage calculation
    V = ljm.eReadName(handle, "AIN3") * (Rth / (Rth+R2)) 
    return V
